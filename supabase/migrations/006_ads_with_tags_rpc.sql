-- Final version of get_ads_with_metrics: includes tags alongside scores.
...
language sql stable security definer as $$
  with metrics as (
    select
      ...
      case when sum(i.impressions) > 0
        then sum(i.spend) / sum(i.impressions) * 1000 else null end as cpm,
      case when sum(i.impressions) > 0
        then sum(i.link_clicks)::numeric / sum(i.impressions) else null end as ctr_all,
      case when sum(i.result_count) > 0
        then sum(i.spend) / sum(i.result_count) else null end as cost_per_result,
      avg(i.frequency)                  as frequency,
      case when sum(i.impressions) > 0
        then avg(i.video_hook_rate) else null end as hook_rate,
      case when sum(i.impressions) > 0
        then avg(i.video_hold_rate) else null end as hold_rate,
      count(distinct i.date)            as days_active
    from ads a
    join adsets s on s.id = a.adset_id
    join campaigns c on c.id = s.campaign_id
    left join ad_insights_daily i
      on i.ad_id = a.id and i.date between p_date_start and p_date_end
  ...
  latest_scores as (
    select distinct on (sc.ad_id) sc.ad_id, sc.score, sc.lifecycle_stage
  ...
  left join ad_tags t on t.ad_id = m.ad_id
